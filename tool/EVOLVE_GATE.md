...
```bash
# 모든 프로젝트 dry-run
python3 shared/tool/evolve_gate.py --dry-run

# 단일 프로젝트
python3 shared/tool/evolve_gate.py --project anima

# 실제 승격 (통과 시 roadmap.json 수정 + evolve_history.jsonl 기록)
python3 shared/tool/evolve_gate.py

# 강제 승격 (MANDATORY check 우회, 위험)
python3 shared/tool/evolve_gate.py --project X --force
```
