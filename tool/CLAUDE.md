...
convention (2026-04-14~ 훅 시스템 대체):
  ...
  smash|free 실행      shared/tool/exec_validated {mode} "{seed}" {engine} {args} (cmd_gate 적용, bin/ 심링크 가능)
...
pending:
  hooks-config.json 등록   gc --weekly 주간 체인 (shared/tool/hooks-config-patch.json 참조)
