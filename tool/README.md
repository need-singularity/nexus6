...
```bash
hexa shared/tool/install-hook.hexa
```
...
```bash
HARNESS_LINT_MODE=all hexa shared/tool/lint.hexa              # 전체 shared/ 스캔 (무거움 — 로컬에서만)
HARNESS_LINT_MODE=file HARNESS_LINT_FILE=shared/xxx.hexa hexa ...  # 단일 파일
HARNESS_GC_WEEKLY_MODE=status hexa shared/tool/gc-weekly.hexa   # 쿨다운 확인
HARNESS_GC_WEEKLY_MODE=force  hexa shared/tool/gc-weekly.hexa   # 쿨다운 무시
```
...
```bash
hexa shared/tool/lint.hexa --staged      # 스테이징된 파일만 (pre-commit이 자동으로 함)
hexa shared/tool/lint.hexa --all         # shared/ 전체
hexa shared/tool/lint.hexa --file 경로    # 특정 파일 하나
```
...
```bash
hexa shared/tool/gc.hexa --scan          # drift + violation + dead 전부
hexa shared/tool/gc.hexa --drift         # 문서↔실제 경로 불일치만
hexa shared/tool/gc.hexa --violation     # 규칙 위반 grep만
hexa shared/tool/gc.hexa --dead          # 90일 미변경 + 미참조 (느림)
hexa shared/tool/gc.hexa --report        # 최근 로그 5줄
```

### 자동 교정 제안 (L2)

```bash
hexa shared/tool/autofix.hexa --analyze                  # mistakes.jsonl 분석
hexa shared/tool/autofix.hexa --analyze --threshold 5    # 반복 기준 (기본 3)
hexa shared/tool/autofix.hexa --analyze --window 200     # 최근 N줄 (기본 500)
hexa shared/tool/autofix.hexa --report                   # 최근 제안
```
...
```bash
hexa shared/tool/gc-weekly.hexa            # 필요하면 실행, 아니면 silent
hexa shared/tool/gc-weekly.hexa --status   # 마지막 실행 / 쿨다운 잔여
hexa shared/tool/gc-weekly.hexa --force    # 쿨다운 무시 강제 실행
```

### Bitter Lesson 게이트

```bash
hexa shared/tool/bitter-gate.hexa --audit             # 규칙별 히트 감사
hexa shared/tool/bitter-gate.hexa --audit --window 500 # 최근 N줄 범위
hexa shared/tool/bitter-gate.hexa --report            # 최근 감사 결과
```
...
```
❌ harness-lint 위반 — shared/tool/lint_log.jsonl 참조
```
