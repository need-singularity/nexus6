...
작업 시작 전 `shared/rules/common.json` + `shared/rules/nexus.json` 을 읽고 전 규칙 준수. 위반 시 즉시 수정.

## blowup AI-native 인프라 (2026-04-11 도입)

- **Phase 1 4단 fast-path** (`graph_stats_single_pass`):
  - exact hit: mtime+size 일치 → O(1) ~83ms
  - incremental: append-only tail 구간만 파싱 → O(Δ lines)
  - shell uplift: `shared/blowup/lib/atlas_stats.awk` → ~0.6s/12MB, 메모리 100MB 이내 (12MB+ atlas hexa-loop 126GB OOM 회피, 2026-04-12 dmesg 확정)
  - hexa fallback: 작은 atlas 만 안전 (legacy)
...
bt/            BreakThrough audit — bt_audit.hexa(R1)/auto_bt.log
...
| `shared/lockdown/l0_guard.hexa` | L0 골화 자동 보호 CLI — verify/sync/merge/status (전 7 프로젝트 공용) |
...
`hexa ~/Dev/nexus/shared/lockdown/l0_guard.hexa <verify|sync|merge|status>` — cwd git 리포 자동 감지, SSOT `shared/lockdown/lockdown.json` projects.{name}.L0 배열로 (1) 파일 존재 (2) CODEOWNERS 동기화 (3) GitHub branch protection (enforce_admins/PR 리뷰/CodeOwner/force-push/delete 차단) 일괄 검증·복구. solo-repo PR 머지는 `merge <PR#>` (enforce_admins OFF → `gh pr merge --admin` → ON 안전 패턴). anima/scripts/l0_guard.hexa (REPO 하드코딩) 의 7 프로젝트 공용 포팅 (2026-04-12). l0-block.hexa PreToolUse 훅은 "평시 자유 수정" 정책 위배로 동시 삭제.
