...
nexus6 mk2는 103개 모듈, 711개 법칙, 72.5k+ 발견을 보유한 HEXA-native 발견 엔진이다.
...
let NEXUS6 = DEV + "/nexus6"
let ENGINE_DIR = NEXUS6 + "/mk2_hexa/native"
let SHARED = NEXUS6 + "/shared"
let SCRIPTS = NEXUS6 + "/scripts"
...
```bash
claude -p "Generate README.md for nexus6. Metrics: modules=[N], laws=[N], discoveries=[N], alien_index=(d=[D],r=[R]), breakthrough_rate=[R]. Style: concise, technical, with emoji section headers. Korean descriptions, English technical terms." --timeout 30000
```
...
| engine_nexus6.hexa | 2 | 프로젝트 경로 → paths.NEXUS6 |
...
nexus6가 스스로 발견하고, 평가하고, 성장하고, 기록하고, 커밋하는
