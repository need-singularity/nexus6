...
| Modify | `shared/reality_map_3d.html` | DSE 공명 그래프 레이어 + growth_bus 이벤트 스트리밍 |
| Create | `shared/dse_graph_3d.html` | DSE 전용 3D 공명 그래프 (독립 뷰) |
...
cross_dse_fusion.hexa가 출력하는 `shared/dse_cross_results.json` 구조 확인:

```bash
ssh hetzner 'head -c 2000 ~/Dev/nexus/shared/dse_cross_results.json' 2>/dev/null || python3 -c "import json;d=json.load(open('shared/dse_cross_results.json'));print(json.dumps(d['top_pairs'][:3],indent=2,ensure_ascii=False))"
```
...
// 입력: shared/dse_cross_results.json (cross_dse_fusion.hexa 출력)
// 출력: shared/dse_joint_results.json + growth_bus.jsonl
//
// Usage:
...
**Files:**
- Create: `shared/dse_graph_3d.html`
- Modify: `shared/reality_map_3d.html`
...
`shared/reality_map_3d.html` 수정 — controls div 끝(`</div>` line 61 부근)에 추가:
...
```bash
git add shared/dse_graph_3d.html shared/reality_map_3d.html
