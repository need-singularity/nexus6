...
```bash
ssh hetzner 'head -c 2000 $NEXUS/shared/dse/dse_cross_results.json' 2>/dev/null || python3 -c "import json;d=json.load(open('shared/dse/dse_cross_results.json'));print(json.dumps(d['top_pairs'][:3],indent=2,ensure_ascii=False))"
```
...
```bash
ssh hetzner '$HEXA_LANG/hexa $NEXUS/mk2_hexa/native/dse_joint_optimizer.hexa --top 5'
